...
class Auth
{
     ...
     * @return boolean
     */
    public function register($openid, $options)
    {
        // 检测是否录入过用户信息
        $where = ['openid'=>$openid,'token'=>$this->token];
        $table_wechat_user = Db::name('wechat_user');
        $userinfo = $table_wechat_user
            ->where($where)
            ->find();
        if(!$userinfo){
            $ip = request()->ip();
            ...
            try
            {
                $user = $table_wechat_user->insert($params);
